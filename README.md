...
│      ▼                                                                     │
│  ┌───────────────────────────────────────────────────────────────────────┐  │
│  │ manager process (codex_manager.py)                                   │  │
│  │                                                                       │  │
│  │  bot_comm (TG handlers)                                               │  │
│  │     │                                                                 │  │
│  │     ▼                                                                 │  │
│  │  service (dispatch/session/approval/token)                            │  │
│  │     │                    ▲                                             │  │
│  │     ▼                    │                                             │  │
│  │  infra.ws_server <───────┘  (register/task_assign/progress/result)    │  │
│  │     │                                                                 │  │
│  │     ▼                                                                 │  │
│  │  infra.db (SQLite: manager_data.db)                                   │  │
│  └───────────────────────────────────────────────────────────────────────┘  │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
               ▲                                            ▲
...
│   - app-server bridge         │               │   - app-server bridge         │
...
注：中间的 `...` 表示可横向扩展为 N 个 Node 节点（Node Host A ... Node Host B）。

### 2) 一次 turn 时序（简化）

```text
User(TG)      Manager(bot+service)      Manager(ws)          Node         codex app-server
   |                  |                      |                 |                 |
1. |----prompt------->|                      |                 |                 |
   |                  | route(chat->node)    |                 |                 |
2. |                  |----task_assign------>|---------------->|                 |
   |                  |                      |                 | enqueue          |
3. |                  |<-----task_ack--------|<----------------|                 |
4. |                  |<---task_progress-----|<----------------|<--item/*...----|
   |<--edit placeholder(progress throttle)---|                 |                 |
5. |                  |                      |                 |--turn/start---->|
   |                  |                      |                 |<--turn/completed|
6. |                  |<----task_result------|<----------------|                 |
   |<--final reply----|                      |                 |                 |
...
注意：systemd 服务不会读取 `~/.bashrc`。  
如果你是手工部署，需要把 `HTTP_PROXY/HTTPS_PROXY/NO_PROXY` 同步进 systemd 的 `EnvironmentFile`（见下文 `scripts/sync_proxy_to_agent_env.sh`）。  
如果你使用 `ansible/deploy_cloud_huang.yml`，playbook 会自动执行同步脚本：先“无代理直连探测 OpenAI 官方接口”，直连可达则跳过，直连不可达才写入代理变量。

Ansible 脱敏约束（重要）：

- 仓库内只保留占位符，不提交真实 `telegram_bot_token`、`manager_control_token`、`node_token`、真实公网地址。
- `ansible/deploy_cloud_huang.yml` 默认不会提供敏感值；部署时必须通过 `-e` 或 `ansible-vault` 注入：
  - `-e manager_control_token='RANDOM-LONG-TOKEN'`
  - 首次写入 TG token 时：`-e manage_telegram_token=true -e telegram_bot_token='123:ABC...'`
- 推荐把敏感变量写到 vault 文件，例如 `ansible/secrets.vault.yml`，执行时用：
  - `ansible-playbook ... -e @ansible/secrets.vault.yml --ask-vault-pass`
