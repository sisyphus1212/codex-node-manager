...
OPENAI_PROBE_URL="${CODEX_SYNC_OPENAI_PROBE_URL:-https://api.openai.com/v1/models}"
OPENAI_PROBE_TIMEOUT="${CODEX_SYNC_OPENAI_PROBE_TIMEOUT:-8}"
...
_can_reach_openai_direct() {
  local code=""
  code="$(
    env -u HTTP_PROXY -u HTTPS_PROXY -u http_proxy -u https_proxy -u ALL_PROXY -u all_proxy \
      curl --noproxy '*' -sS -o /dev/null -w '%{http_code}' \
      --max-time "$OPENAI_PROBE_TIMEOUT" \
...
if _can_reach_openai_direct; then
  echo "Direct OpenAI access is available (${OPENAI_PROBE_URL}); skip syncing proxy env."
  exit 0
fi
...
echo "Synced proxy env to ${ENV_FILE}"
